...
const AddMetrixData: React.FC<AddMetrixDataProps> = ({ medicalData, weightKg, setWeightKg, setIsModified, heightFeet, setHeightFeet, heightInches, setHeightInches, bmi, handleSaveMetrics, isModified, showDropdown, setShowDropdown, setSelectedUser, selectedUser, memberList }) => {
  ...
  return (
    <div className="relative flex flex-col items-center text-center w-full lg:w-1/3 xl:w-1/2 2xl:w-1/3 bg-blue-50 p-3 sm:p-4 lg:p-6 rounded-lg shadow-md">
        ...
        {showDropdown && (
          <div className="absolute z-10 mt-2 w-44 bg-white border border-gray-300 rounded-lg shadow-md">
            <div
              ...
              className={`px-4 py-2 cursor-pointer hover:bg-gray-100 ${selectedUser === "all" ? "bg-gray-100 font-medium" : ""
                }`}
            >
              All Users
            </div>
            {memberList?.map((member: any) => (
              <div
                ...
                className={`px-4 py-2 cursor-pointer hover:bg-gray-100 ${selectedUser === member.id.toString() ? "bg-gray-100 font-medium" : ""
                  }`}
              >
                {member.firstName}
              </div>
